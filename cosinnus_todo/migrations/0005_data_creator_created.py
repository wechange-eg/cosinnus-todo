...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for entry in orm.TodoEntry.objects.all():
            entry.creator = entry.created_by
            entry.created = entry.created_date
            entry.save()

    def backwards(self, orm):
        for entry in orm.TodoEntry.objects.all():
            entry.created_by = entry.creator
            entry.created_date = entry.created
            entry.save()
